...
//		numOfAccounts = 0;
	...
	void makeAccount(int choice) {
		...
		if(choice==1) {
			...
			System.out.print("기본이자%(정수형태로입력): ");mInterest = scanner.nextInt();
			NormalAccount normal = new NormalAccount
					(iAcnumber, iName, iMoney, mInterest);
			...
			System.out.print("잔고: ");iMoney = scanner.nextInt();
			System.out.print("기본이자%(정수형태로입력): ");mInterest = scanner.nextInt();
			System.out.print("신용등급(A,B,C등급): ");cRating = scanner.nextLine();
			System.out.print("신용등급(A,B,C등급): ");cRating = scanner.nextLine();
			HighCreditAccount high = new HighCreditAccount
					(iAcnumber, iName, iMoney, mInterest, cRating);
